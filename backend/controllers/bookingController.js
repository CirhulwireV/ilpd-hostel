...
const hasAvailableRoom = async (category, checkIn, checkOut, requiredCount = 1, hostelSection) => {
  const rooms = await getAvailableRoomsForDates(category, checkIn, checkOut, hostelSection);
...
// Step 1: Client selects block (+ optional category) + dates, pays the full amount via Stripe.
exports.createBooking = async (req, res) => {
  const { category, blockName, hostelSection, checkIn, checkOut } = req.body;
  const requestedOccupants = Math.min(20, Math.max(1, Number(req.body.numberOfOccupants || 1)));
  const occupantNames = Array.isArray(req.body.occupantNames)
    ? req.body.occupantNames.slice(0, requestedOccupants).map((n, i) => String(n || `Occupant ${i + 1}`).trim() || `Occupant ${i + 1}`)
    : [];
  const selectedBlock = String(blockName || hostelSection || "").trim();
  ...
  try {
    ...
    if (!selectedBlock) {
      return res.status(400).json({ message: "Please select an accommodation block." });
    }

    const rateInfo = await getBookingRate({ blockName: selectedBlock, category });
    if (!rateInfo) {
      return res.status(400).json({ message: `No price is configured for ${selectedBlock}${category ? " / " + category : ""}. Please contact the admin.` });
    }
    const { rate: baseRate, billingPeriod, usesCategories, categoryLabel } = rateInfo;
    ...
    if (availableRooms.length < requestedOccupants) {
      return res.status(409).json({
        message: availableRooms.length
          ? `Only ${availableRooms.length} room(s) are available in ${selectedBlock} for these dates, but ${requestedOccupants} occupant(s) were requested.`
          : `No rooms are available in ${selectedBlock} for these dates. Please choose another block or contact the admin.`
      });
    }
    ...
    const totalPrice = billingPeriod === "night"
      ? baseRate * stayDays * requestedOccupants
      : baseRate * billingMonths * requestedOccupants;
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      line_items: [{
        price_data: {
          currency: "rwf",
          product_data: {
            name: `${categoryLabel} Room - ${selectedBlock}`,
            description: `${selectedBlock} | ${categoryLabel} | ${requestedOccupants} occupant(s) | ${billingPeriod === "night" ? `${stayDays} night(s)` : `${billingMonths} month${billingMonths !== 1 ? "s" : ""}`} | Rooms assigned by admin after payment`,
          },
...
exports.confirmBooking = async (req, res) => {
  ...
  try {
    ...
    const { category, blockName, billingPeriod, clientId, checkIn, checkOut, billingMonths, monthlyRate, totalPrice, numberOfOccupants, occupantNames: occupantNamesRaw } = md;
    ...
    if (!blockName) return res.status(400).json({ message: "Stripe booking metadata is incomplete" });
    ...
    if (!checkIn || !checkOut || !totalPrice || !monthlyRate) {
    ...
    const expectedTotal = expectedPeriod === "night"
      ? expectedRate * expectedDays * occupantCount
      : expectedRate * expectedMonths * occupantCount;

    if (
      receivedMonths !== expectedMonths ||
      receivedTotal !== expectedTotal ||
      session.amount_total !== expectedTotal ||
      session.currency !== "rwf"
    ) {
    ...
    const booking = await Booking.create({
      ...
      occupants: Array.from({ length: occupantCount }, (_, i) => ({
        name: (() => { try { const a = JSON.parse(occupantNamesRaw || "[]"); return String(a[i] || `Occupant ${i + 1}`); } catch { return `Occupant ${i + 1}`; } })(),
...
exports.handleStripeWebhook = async (req, res) => {
  ...
  if (event.type === "checkout.session.completed") {
    ...
    if (session.payment_status === "paid") {
      ...
      if (metadata.type === "booking_update") {
        try {
          ...
          if (booking && !alreadyApplied && booking.status === "pending" && !booking.room && booking.client.toString() === String(metadata.clientId) && Number.isFinite(newTotal) && Number.isFinite(additionalAmount) && additionalAmount > 0 && session.amount_total === additionalAmount && session.currency === "rwf") {
            ...
            if (expectedTotal === newTotal && newTotal - Number(booking.totalPrice) === additionalAmount && (await hasAvailableRoom(booking.category, checkIn, checkOut, Number(booking.numberOfOccupants || booking.occupants?.length || 1), booking.blockName))) {
              booking.checkIn = checkIn;
              booking.checkOut = checkOut;
              booking.billingMonths = billingPeriod === "month" ? calculateBillingMonths(checkIn, checkOut) : 0;
      ...
      const { category, blockName, billingPeriod, clientId, checkIn, checkOut, billingMonths, monthlyRate, totalPrice, numberOfOccupants, occupantNames: occupantNamesRaw } = metadata;
      ...
      const expectedTotal = expectedPeriod === "night"
        ? expectedRate * expectedDays * occupantCount
        : expectedRate * expectedMonths * occupantCount;
      const validPaymentMetadata = expectedRate && Number(billingMonths) === expectedMonths && billingPeriod === expectedPeriod && Number(totalPrice) === expectedTotal && session.amount_total === expectedTotal && session.currency === "rwf";
      if (clientId && blockName && validPaymentMetadata) {
        try {
...
// ─────────────────────────────────────────────────────────────────────────────
// Everything below is unchanged from your original file. Only the pricing
// source changed above. This includes all admin/check-in/check-out/report
// handlers.
// ─────────────────────────────────────────────────────────────────────────────
...
exports.reallocateRoom = async (req, res) => {
  ...
  try {
    ...
    if (!newRoom) return res.status(409).json({ message: "The selected room is not available (wrong block, category, or already booked)." });
...
exports.allocateAndConfirm = async (req, res) => {
  ...
  try {
    ...
    const availableRooms = await getAvailableRoomsForDates(booking.category, booking.checkIn, booking.checkOut, booking.blockName);
...
exports.getBookingPolicy = async (req, res) => {
  res.json({
    ...
    refundRule: `For a pending booking reduction or an early admin-recorded checkout, unused paid time is refunded using the accommodation tariff: monthly stays use monthly rate / ${REFUND_DAILY_DIVISOR} per day, while per-night stays use the full nightly rate per unused night. Each occupant is calculated separately. Refunds are automatic when Stripe can process them; otherwise they are marked refund_pending for admin/manual processing.`,
...
exports.updatePendingBooking = async (req, res) => {
  ...
  try {
    ...
    const available = await getAvailableRoomsForDates(booking.category, checkInDate, checkOutDate, booking.blockName);
    if (available.length < occupantCount) return res.status(409).json({ message: `Only ${available.length} room(s) are available for the new dates, but this booking needs ${occupantCount}.` });
    ...
    if (difference > 0) {
      const session = await stripe.checkout.sessions.create({
        ...
        line_items: [{
          price_data: {
            currency: "rwf",
            product_data: { name: `Booking extension - ${booking.blockName || ""} ${booking.category || ""}`.trim(), description: `Additional stay payment for booking ${booking._id}` },
...
exports.confirmBookingUpdate = async (req, res) => {
  ...
  try {
    ...
    if (!hasRoom) {
      if (!(await hasAvailableRoom(booking.category, checkIn, checkOut, Number(booking.numberOfOccupants || booking.occupants?.length || 1), booking.blockName))) return res.status(409).json({ message: "There are not enough rooms available for all people in this booking for the new dates." });
    } else {
      const overlap = await Booking.findOne({
        _id: { $ne: booking._id },
        room: booking.room,
    ...
    booking.checkIn = checkIn;
    booking.checkOut = checkOut;
    const updateBillingPeriod = booking.billingPeriod || "month";
...
exports.getMonthlyReport = async (req, res) => {
  try {
    ...
    for (const b of bookings) {
      ...
      const block = b.room?.hostelSection || b.blockName || "Unknown";
      revenueByCategory[category] = (revenueByCategory[category] || 0) + net;
      revenueByBlock[block] = (revenueByBlock[block] || 0) + net;
